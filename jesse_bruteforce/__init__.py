...
logger.addHandler(logging.FileHandler("jesse-bruteforce.log", mode="w"))
...
@click.group()
@click.version_option(pkg_resources.get_distribution("jesse-bruteforce").version)
def cli() -> None:
    pass
...
def create_config() -> None:
    ...
    shutil.copy2(f'{package_dir}/bruteforce-config.yml', f'{target_dirname}/bruteforce-config.yml')
...
def run() -> None:
    ...
    path = f'storage/jesse-bruteforce/csv/{study_name}.csv'
    os.makedirs('./storage/jesse-bruteforce/csv', exist_ok=True)
    ...
    if cfg['optimizer'] in ["EvolutionStrategyOptimizer", "RandomRestartHillClimbingOptimizer", "StochasticHillClimbingOptimizer"]:
        ...
        if True:
            ...
            mem.drop([f'testing_{k}' for k in empty_backtest_data.keys()], 1, inplace=True)
...
def get_config():
    cfg_file = pathlib.Path('bruteforce-config.yml')

    if not cfg_file.is_file():
        print("bruteforce-config.yml not found. Run create-config command.")
        exit()
    else:
        with open("bruteforce-config.yml", "r") as ymlfile:
...
def get_candles_with_cache(exchange: str, symbol: str, start_date: str, finish_date: str) -> np.ndarray:
    path = pathlib.Path('storage/jesse-bruteforce')
    path.mkdir(parents=True, exist_ok=True)

    cache_file_name = f"{exchange}-{symbol}-1m-{start_date}-{finish_date}.pickle"
    cache_file = pathlib.Path(f'storage/jesse-bruteforce/{cache_file_name}')

    if cache_file.is_file():
        with open(f'storage/jesse-bruteforce/{cache_file_name}', 'rb') as handle:
            candles = pickle.load(handle)
    else:
        candles = get_candles(exchange, symbol, '1m', start_date, finish_date)
        with open(f'storage/jesse-bruteforce/{cache_file_name}', 'wb') as handle:
